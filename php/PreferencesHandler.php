...
//                    "time_zone_offset" => array("default" => "2", "label" => "Zeitzonenverschiebung (im Vergleich zur UTC) in Stunden", "type" => "number"),
                    "userpolls_result_length" => array("default" => "3", "label" => "Anzahl der Personen die pro Umfrage in der Ergebnisliste angezeigt werden", "update_func" => function($val) {
                            Poll::updateAll();
                        }),
    ...
    public function post() {
        if (Auth::canModifyPreferences()) {
            foreach ($this->pref_vals as $key => $value) {
                $old = $this->pref_vals[$key]["default"];
                ...
                if ($old != $this->pref_vals[$key]["default"] && isset($this->pref_vals[$key]["update_func"])) {
                    $this->pref_vals[$key]["update_func"]($old, $this->pref_vals[$key]["default"]);
                }
            }
            $this->updateDB();
            global $env;
