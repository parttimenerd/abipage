...
class Database {
    ...
    public static function setup() {
        ...
        self::$db->query('CREATE TABLE IF NOT EXISTS ' . DB_PREFIX . "user_characteristics(id INT AUTO_INCREMENT PRIMARY KEY, userid INT, type TINYINT, topic INT, time TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, text TEXT, FULLTEXT(text)) ENGINE = MYISAM") or die("Can't create user_characteristics table: " . self::$db->error);
        ...
        self::$db->query('CREATE TABLE IF NOT EXISTS ' . DB_PREFIX . "poll_answers(id INT AUTO_INCREMENT PRIMARY KEY, userid INT, pollid INT, type TINYINT, answer TEXT, FULLTEXT(answer)) ENGINE = MYISAM") or die("Can't create poll_answers table: " . self::$db->error);
        ...
        self::$db->query('CREATE TABLE IF NOT EXISTS ' . DB_PREFIX . "images(id INT AUTO_INCREMENT PRIMARY KEY, userid INT, description TEXT, category VARCHAR(100), capture_time BIGINT, format VARCHAR(5), time BIGINT, rating FLOAT NOT NULL, rating_count INT NOT NULL, data TEXT, FULLTEXT(description)) ENGINE = MYISAM") or die("Can't create pictures table: " . self::$db->error);
